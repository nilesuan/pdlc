...
1. Invoke `bash scripts/verify-artifact.sh <path>`. Capture stdout and exit code.
...
- Spawn the agent in a non-blocking write posture - `mode: "acceptEdits"`, or pre-granted `Write` + `Edit` in its permission scope (`bypassPermissions` for worktree-isolated agents). A writing agent in a prompting mode can surface a `Write`/`Edit` approval that nobody answers in a background run; the call blocks forever, the agent returns nothing, and the whole pass hangs (LESSON-0001). A blocked write is not a terminal error, so only this posture - plus the Rule #4 deadline below - prevents the hang. See [`../standards/operations/SCHEDULED_WORK.md`](../standards/operations/SCHEDULED_WORK.md).
...
- The pass-loop pattern is the dominant orchestration shape from `~/.claude.old/`'s 6+ months of dogfooding (see [`../../SYSTEM.md`](../research/sources/SYSTEM.md), Section 3, "Workflow system end-to-end").
