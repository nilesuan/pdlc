...
When you `REJECT` a finding, also write a candidate lesson stub at `lessons/<YYYY>/LESSON-NNNN-candidate.md` per [`../standards/process/LEARNING.md`](../standards/process/LEARNING.md) §"What counts as a mistake". `<YYYY>` is the current calendar year; `NNNN` is the next zero-padded sequence number for that year (read existing files in the directory to determine it).
...
- The cross-verifier pattern is the single most high-leverage hallucination defense documented in [`../../SYSTEM.md`](../research/sources/SYSTEM.md), Section 3 ("Cross-verifier as hallucination killer").
