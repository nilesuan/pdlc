...
3. **Hotspot list is the priority order.** Repayment targets the hotspot list, not whichever module the loudest engineer dislikes. The hotspot list is regenerated at least quarterly and shared with the team. The regeneration (the churn × complexity export in §"Hotspot analysis") is a `/schedule` candidate - run it on a fixed cadence rather than relying on someone remembering; see [`../operations/SCHEDULED_WORK.md`](../operations/SCHEDULED_WORK.md).
