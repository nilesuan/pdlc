...
On any of these, write a candidate lesson stub at `lessons/<YYYY>/LESSON-NNNN-candidate.md` per [`process/LEARNING.md`](process/LEARNING.md) §"Lesson file structure". Frame it blamelessly — describe the failure mode, not the agent. The orchestrator promotes the candidate after user review; auto-promotion is forbidden.
