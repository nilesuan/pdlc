...
7. **A writing sub-agent is spawned where it cannot block on a permission prompt, and empty output past the deadline is a failure, not a wait.** Spawn writing sub-agents in a non-blocking write posture (`acceptEdits` / pre-granted `Write` + `Edit` / `bypassPermissions` for worktree). Treat a sub-agent that returns nothing by its Rule #4 deadline as failed: stop and report per the project Subagent Failure Handling rule, never continue waiting. *Why: this was written when an unanswerable approval prompt blocked forever (LESSON-0001). That no longer reproduces: on Claude Code 2.1.235 a write without permission returns promptly with an explicit "needs approval" message instead of hanging - verified by probe under `--permission-mode manual` in a non-interactive run, exiting well inside a 150s kill deadline. The deadline is therefore defence-in-depth now rather than the only recovery. Both halves still earn their place: the non-blocking posture avoids a wasted round-trip, and the deadline still covers an agent stalled for reasons other than a permission prompt. [The spawned-sub-agent-inside-a-session path was not separately probed - UNVERIFIED.]*
