...
> **Scope:** These rules govern work **inside `research/`** — authoring and maintaining the cited evidence corpus. The global operating rules for the whole `~/.claude` system live in the root [`../CLAUDE.md`](../CLAUDE.md). When you are writing or editing research documents, this file wins.

The `research/` tree is a **research corpus**, not a code project. The goal is to document the industry-grade Product Development Life Cycle for software (ideation → production → maintenance → iteration). See `VISION.md`.
